...
public class DictionaryDaoImpl implements DictionaryDao {
    ...
    private static final String GET_PASSPORT = "SELECT * " +
            "FROM jc_passport_office WHERE ps_office_area_id = ?";
    private static final String GET_REGISTER = "SELECT * " +
            "FROM jc_register_office WHERE reg_office_area_id = ?";
        ...
        try (Connection con = getConnection();
            ...
            while (rs.next()) {
                PassportOffice str = new PassportOffice(
                        rs.getLong("ps_office_id"),
                        rs.getString("ps_office_area_id"),
                        rs.getString("ps_office_name"));
                result.add(str);
            }
        } catch (SQLException ex) {
            throw new DaoException(ex);
        ...
        try (Connection con = getConnection();
            ...
            while (rs.next()) {
                RegisterOffice str = new RegisterOffice(
                        rs.getLong("reg_office_id"),
                        rs.getString("reg_office_area_id"),
                        rs.getString("reg_office_name"));
                result.add(str);
            }
        } catch (SQLException ex) {
            throw new DaoException(ex);
